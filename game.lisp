...
(loop
  ...
  (xlib:event-case (display :timeout 0.01)
    (:key-press (code)
      (cond ((equal code 25)(when (= (elt (elt foo (- player-y 1)) player-x) 0)(setf player-y (- player-y 1))))
            ((equal code 38)(when (= (elt (elt foo player-y) (- player-x 1)) 0)(setf player-x (- player-x 1))))
            ((equal code 39)(when (= (elt (elt foo (+ player-y 1)) player-x) 0)(setf player-y (+ player-y 1))))
            ((equal code 40)(when (= (elt (elt foo player-y) (+ player-x 1)) 0)(setf player-x (+ player-x 1))))
            ((equal code 45)(progn (stun-hector)(setf spell-display (slice (write-to-string spells)))))
      )
      t
    )
  )
